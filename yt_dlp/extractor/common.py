...
class InfoExtractor:
    ...
    def _parse_mpd_periods(self, mpd_doc, mpd_id=None, mpd_base_url='', mpd_url=None):
        ...
        def extract_drm_info(adaptation_set):
            """Extract Widevine PSSH and KID from ContentProtection."""
            drm_info = {}
            content_protections = adaptation_set.findall(_add_ns('ContentProtection'))

            for cp in content_protections:
                scheme_id_uri = cp.attrib.get('schemeIdUri', '').lower()
                if scheme_id_uri == 'urn:mpeg:dash:mp4protection:2011':
                    drm_info['kid'] = cp.attrib.get('{urn:mpeg:cenc:2013}default_KID')  # Extract KID
            return drm_info
        ...
        for period_idx, period in enumerate(mpd_doc.findall(_add_ns('Period'))):
            ...
            for adaptation_set in period.findall(_add_ns('AdaptationSet')):
                ...
                drm_info = extract_drm_info(adaptation_set)  # Extract DRM info for the AdaptationSet
                ...
                for representation in adaptation_set.findall(_add_ns('Representation')):
                    ...
                    if drm_info:  # Include PSSH and KID in the format if present
                        f.update({
